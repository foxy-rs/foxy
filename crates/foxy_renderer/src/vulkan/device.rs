...
impl FoxyDevice {
  pub fn new(instance: Arc<Instance>, surface: Arc<Surface>) -> Result<Self, VulkanError> {
...
impl FoxyDevice {
  fn pick_physical_device(
  ...
  ) -> Result<(Arc<PhysicalDevice>, u32), VulkanError> {
    ...
    let (physical, queue_family_index) = physical_devices
      ...
      .ok_or_else(|| VulkanError::NoValidDevice)?;
  ...
  ) -> Result<(Arc<Device>, Arc<Queue>), VulkanError> {
